"""Acceptance test for Phase II env scaffolding v2_extended_history."""
...
def run_episode(core_id: str, policy: str, seed: int = 42) -> RunSummary:
    ...
    while True:
        ...
        if len(obs) != 3:
            raise AssertionError(f"observation length is {len(obs)}, expected 3")

        if obs[2] != 0.0:
            all_trend_zeros = False

        if core_id == "730d_core" and steps < 10:
            is_nan, is_inf = _is_bad(float(obs[2]))
            if is_nan or is_inf:
                raise AssertionError(f"First 10 steps of 730d_core must produce a valid eg_p_trend: failed at step {steps} with {obs[2]}")

        if steps in (10, 50, 100):
            # Check numerical consistency
            closes = env.loader.close_panel()
            # obs was computed using the timestamp before env._step_idx was incremented
            current_ts = env._episode_timestamps[steps - 1]
            ctx = env._make_context(current_ts)
            p_t = _compute_eg_pvalue_at(ctx, ctx.timestamp)
            window = slice_trailing_bars(closes.index, ctx.timestamp, "21d", ctx.bar_frequency)
            p_past = _compute_eg_pvalue_at(ctx, window[0])
            expected_trend = p_t - p_past
            if not math.isclose(obs[2], expected_trend, abs_tol=1e-9):
                raise AssertionError(f"trend mismatch at step {steps}: obs {obs[2]} != computed {expected_trend}")
    ...
    if all_trend_zeros:
        raise AssertionError(f"eg_p_trend is constantly zero across the episode pass on core {core_id}")
...
def main() -> None:
    ...
    for core_id in EPISODE_CORES:
        for policy in ("noop", "random"):
            ...
            header = (
                f"# env_scaffolding_v2_extended_history acceptance run\n"
    ...
    combined_lines = [
        "# env_scaffolding_v2_extended_history combined acceptance summary",
        f"# generated_at_utc: {datetime.now(timezone.utc).isoformat()}",
        f"# snapshot_id: {SNAPSHOT_ID}",
        "",
    ]
