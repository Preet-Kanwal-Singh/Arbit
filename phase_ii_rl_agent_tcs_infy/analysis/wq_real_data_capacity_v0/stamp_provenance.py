"""Provenance stamper for Tier B real-data capacity check."""
...
def main():
    # Only claim the specific files for this phase/check
    output_files = sorted(OUT_DIR.glob("*.json")) + sorted(OUT_DIR.glob("*.md")) + sorted(OUT_DIR.glob("*.py"))
    output_files += sorted(OUT_DIR.glob("tb_real/**/events.out.tfevents.*"))
    output_files += sorted(OUT_DIR.glob("best_model.zip"))
    output_files = [f for f in output_files if f.name != "provenance.json"]

    provenance = {
        "claim_id": "wq_real_data_capacity_v0",
        "phase": "phase_ii_rl_agent_tcs_infy",
        "tier": "B",
        "snapshot_id": "tcs_infy_v4_2026-07-13",
        "synthetic": False,
        "script_path": str((OUT_DIR / "train_real_data.py").relative_to(ROOT)).replace("\\", "/"),
