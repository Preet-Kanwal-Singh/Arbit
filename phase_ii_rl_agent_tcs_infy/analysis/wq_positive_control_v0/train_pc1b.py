"""Train PPO on SyntheticOracleEnv with differential_sharpe."""
...
def main():
    ...
    tb_log_dir = out_dir / "tb_pc1b"
    
    refs_path = out_dir / "oracle_refs.json"
    refs = json.loads(refs_path.read_text())
    p99_threshold = refs["dsr_p99_threshold"]
    ...
    # Shadow eval env is ALWAYS cost_adjusted_pnl, as requested
    eval_kwargs = env_kwargs.copy()
    eval_kwargs["reward_name"] = "cost_adjusted_pnl"
    eval_env = GymSyntheticOracleEnv(**eval_kwargs)
    ...
    print("Starting PC-1b training (differential_sharpe)...")
